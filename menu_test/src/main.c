...
game_server_t *make_server_online_component(int chosen_port)
{
	game_server_t *serv = malloc(sizeof(game_server_t));
	if (!serv)
		return (NULL);
	serv->addr = init_server(&serv->sock, chosen_port);
...
game_client_t *make_client_online_component(int *player_number, int chosen_port)
{
	game_client_t *serv_access = malloc(sizeof(game_client_t));
	if (!serv_access)
		return (NULL);
	serv_access->server_access = init_client(&serv_access->server_socket, player_number, chosen_port);
...
int prepare_systems(game_t **game, int player_type, int chosen_port)
{
	void *online_component = NULL;
	int	player_number = -1;

	if (player_type == 0) {
		online_component = (void *)make_server_online_component(chosen_port);
		player_number = 0;
	} else
		online_component = (void *)make_client_online_component(&player_number, chosen_port);
	if (!online_component){
...
int main_game_loop(int player_type, int chosen_port)
{
	...
	if (prepare_systems(&game, player_type, chosen_port) > 0)
		return (84);
